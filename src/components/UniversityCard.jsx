 ...
 * @param {string|number} props.id - University ID
 */
const UniversityCard = ({
  ...
  return (
    <Link to={`/university/${id || 'coventry'}`} className="block h-full">
        ...
        <div className={`${isListView ? 'flex flex-col md:flex-row md:items-center' : 'flex flex-col'} p-4 h-full`}>
          ...
          <div className={`${isListView ? 'md:w-48 flex-shrink-0' : 'w-full'}`}>
            <button 
              ...
              className={`bg-[#05213b] text-white py-3 px-4 rounded-xl font-medium text-sm hover:bg-[#1a3450] transition-all flex items-center justify-center gap-3 group w-full ${isListView ? 'md:mt-0' : 'mt-4'}`}
            >
